...
class Program:
    def __init__(self, args:dict) -> None:
        ...
        self.records_group_prefix = "/svs/records"
        self.svs:SVSync = SVSync(app, Name.from_str(self.group_prefix), Name.from_str(self.args["node_id"]), self.missing_callback)
        self.svs_records = SVSync(app, Name.from_str(self.records_group_prefix), Name.from_str(self.args["node_id"]), None)
        self.records_list = []
        print(f'SVS Consumer started! | group prefix: {self.group_prefix}, records group prefix: {self.records_group_prefix},  node id: {self.args["node_id"]} |')
